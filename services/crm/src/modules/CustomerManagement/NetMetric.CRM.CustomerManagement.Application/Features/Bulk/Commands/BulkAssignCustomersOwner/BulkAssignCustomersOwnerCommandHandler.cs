...
public sealed class BulkAssignCustomersOwnerCommandHandler(
    ICustomerManagementDbContext dbContext,
    ICurrentUserService currentUserService) : IRequestHandler<BulkAssignCustomersOwnerCommand, BulkOperationResultDto>
{
    private readonly ICustomerManagementDbContext _dbContext = dbContext;
    private readonly ICurrentUserService _currentUserService = currentUserService;

    public async Task<BulkOperationResultDto> Handle(BulkAssignCustomersOwnerCommand request, CancellationToken cancellationToken)
    {
        _currentUserService.EnsureAuthenticated();
        var tenantId = _currentUserService.TenantId;
        var actor = _currentUserService.UserName ?? "SYSTEM";
        var now = DateTime.UtcNow;
        var ids = request.CustomerIds.Where(x => x != Guid.Empty).Distinct().ToList();

        var entities = await _dbContext.Set<Customer>()
        ...
        return new BulkOperationResultDto
        {
            ...
            Message = "Customer owner assignment completed."
        };
    }
}
